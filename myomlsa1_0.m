...
clear
[y_in_orig, fs0] = audioread('NOISY_fileid_0.wav');
fs = 16e3;
...
while(loop_i+frame_length<data_length)
    ...
    if(loop_i==1)   % initialization
        ...
        gamma = 1;  % instant SNR estimation                 houyanSNR
    ...
    eta = alpha_eta * eta_2term + (1-alpha_eta) * max(gamma-1, 0);  % update smoothed SNR, eq.18, where eta_2term = GH1 .^ 2 .* gamma % xianyanSNR
    eta = max(eta, eta_min);
    v = gamma .* eta ./ (1+eta); % v 
